...
Entity::~Entity() { delete this->_movement; }
...
void Entity::update(const float& dt) {
    this->_movement->update(dt);   // move
    this->animate();               // animate
    // when entity collides with window border, animation finishes
    if (this->_checkCollision()) this->_hasAnimationFinished = true;
}
...
bool Entity::_checkCollision() {
    ...
    hitbox["LEFT"] = this->_shape->getGlobalBounds().left;
    hitbox["RIGHT"] = this->_shape->getGlobalBounds().left +
                      this->_shape->getGlobalBounds().width;
    hitbox["UP"] = this->_shape->getGlobalBounds().top;
    hitbox["DOWN"] = this->_shape->getGlobalBounds().top +
                     this->_shape->getGlobalBounds().height;
    // checks if entity hitbox collides with window border and/or menu
    return !(hitbox["LEFT"] > 0 && hitbox["RIGHT"] < WINDOW_WIDTH &&
             hitbox["UP"] > 0 &&
             hitbox["DOWN"] < WINDOW_HEIGHT - MENU_BAR_HEIGHT);
}

float Entity::_getRandomDirection() {
    float diff =
        1.0 - 0.3;   // define difference between min and max values possible
    // return a float between 0.3 and 1 multiplied either by -1 or 1
    // this ensures that the movement is never 'too slow'
    return (((static_cast<float>(rand()) / RAND_MAX) * diff) + 0.3) *
           (rand() % 2 == 0 ? 1 : -1);
}
